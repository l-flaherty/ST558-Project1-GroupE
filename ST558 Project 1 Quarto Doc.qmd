...
plot.state(combined_state_data = data_combined[[2]], what_var_name = value)
```
...
plot.county <- function(combined_county_data, what_var_name = value, which_state = "NC",  top_or_bottom = "top", how_many_obs = 5){
  ...
  ggplot(plot_state, aes(x = year, y = {{what_var_name}}, color = area_name)) +
    geom_line() +
    labs(x = "Year",
         y = "Enrollment Value",
         color = "Counties",
...
plot.county(combined_county_data = data_combined[[1]], which_state = "CA", what_var_name = value, top_or_bottom = "bottom", how_many_obs = 10)
